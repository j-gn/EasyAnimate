...
{  
	...
	public struct State<T> : IState {
		...
		public float duration; //length of animation in seconds
		...
		public bool Update( float pCurrentTime ){
			...
			if ((StartTime + duration) <= pCurrentTime) { //the animation has ended
				return false;
			}
	...
	internal void Update( float pDeltaTime = float.MinValue ){
		if (pDeltaTime == float.MinValue){
			pDeltaTime = Time.deltaTime;
		}
		for( int i = _animStates.Count -1 ; i >= 0; i--){
			var state = _animStates[i].state;
			if (state.Paused) { 
				state.StartTime += pDeltaTime;
			}
